...
switch ($method) {
    case 'GET':
        $stmt = $db->query('SELECT * FROM kontostand ORDER BY datum DESC');
    ...
    case 'POST':
        ...
        $stmt = $db->prepare('INSERT INTO kontostand (betrag, datum, bemerkung) VALUES (?, ?, ?)');
        $stmt->execute([
            $data['betrag'],
            $data['datum'],
            $data['bemerkung'] ?? null
    ...
    case 'DELETE':
        ...
        $stmt = $db->prepare('DELETE FROM kontostand WHERE id = ?');
        $stmt->execute([$id]);
